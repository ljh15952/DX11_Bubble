...
>
> 최종 갱신: 2026-09-10 / 커밋 `d2b2642` 이후 (5-e-4, 미커밋)
...
| # | 내용 |
|---|---|
| **6** | **JSON 데이터 외부화 + 무브셋(웅크려 공격 등)** ← 여기서 이어간다 |
| 7 | Room 그래프 맵 관리 |
| 8 | 콘텐츠 확장 |

**6단계의 핵심**: 지금까지 `constexpr` 로 박아 둔 것들(AttackData / RollData / HurtData /
ArmorData / PartBox / AnimationClip)이 전부 JSON 으로 빠진다. 구조는 이미 그 형태로
만들어져 있으므로 옮기는 작업이지 설계하는 작업이 아니다.
그리고 **무브셋**(웅크려 공격 등)이 design.md §3.2.1 대로 들어간다.
