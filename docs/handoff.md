...
>
> 최종 갱신: 2026-09-10 / 커밋 `bbaf752` 이후 (6-a·b 무브셋, 미커밋)
...
| 파일 | 내용 |
|---|---|
| `assets/textures/player.png` | 384x448. 6열 x 7행 = idle(4) / run(6) / attack=light(6) / roll(6) / **crouch(6) / thrust(6) / runatk(6)** |
...
| **Ctrl** / 패드 LB | **웅크리기**(누르고 있는 동안). 이동 0.45배 + 공격이 다리 높이로 |
...
| # | 내용 |
|---|---|
| **6-c** | **JSON 외부화 + 핫 리로드** ← 여기서 이어간다 |
| 6-d | 무브셋 스프라이트(웅크리기·찌르기). 구조가 재밌으면 |
| 7 | Room 그래프 맵 관리 |
| 8 | 콘텐츠 확장 |

**6-c 의 핵심**: 지금까지 `constexpr` 로 박아 둔 것들(AttackData / RollData / HurtData /
