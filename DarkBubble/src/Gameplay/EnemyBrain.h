...
public:
    ...
    // ------------------------------------------------------------------------
    //  SetYieldRoom — 「나보다 가까운 동료가 있다」
    //
    //    ★ 적이 여럿이 되면 전부 같은 지점으로 몰려 **한 점에 겹친다.**
    //      몸끼리 충돌을 붙이는 것이 정답처럼 보이지만, 그건 별도 시스템이고
    //      AI 도 커진다. 여기서는 **한 칸 물러서게** 하는 것으로 충분하다.
    //
    //    ★★ 「멈추는 거리」만 늘리고 **공격 사거리는 건드리지 않는다.**
    //      둘을 같이 늘리면 뒤에 선 적이 **닿지도 않는 거리에서 휘두른다.**
    //      물러선 적은 그냥 **기다린다** — 앞의 적이 죽거나 비키면
    //      이 깃발이 꺼지고 그때 들어온다.
    //
    //    ※ 누가 더 가까운지는 Scene 이 판단한다. 적이 서로를 알 필요가 없다 —
    //      SetTargetProne 과 같은 구조다.
    // ------------------------------------------------------------------------
    void SetYieldRoom(bool v) { m_yieldRoom = v; }
