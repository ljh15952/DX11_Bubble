...
namespace
{
    ...
    //
    //   ★★ **가장 높은 점이 25 다.** 이것이 규칙이 되었다 —
    //     적의 휘두르기(SWING)가 발끝 26~36 을 덮으므로, 엎드리면 그 공격이
    //     **몸 위로 지나간다.** 「엎드리면 상단 공격을 흘린다」가 좌표만으로
    //     성립한다. 특수 규칙도 상태 검사도 없다.
    //
    //     그래서 이 표의 top 값을 올릴 때는 SWING 의 heightFromFoot 도 같이
    //     봐야 한다. 둘이 맞물려 있다 (EnemyBrain.cpp 의 kSwing 주석 참조).
    constexpr PartBox kBoxCrawl[Part_Count] = {
        {   4.0f, -25.0f,  20.0f, -11.0f },   // HEAD  — 앞으로 나온다. ★ 그림(11~25)에 맞춘다
    ...
    //
    //   ★★ 가장 높은 점이 **27** 이다 — 전용 그림(10행)의 실루엣과 같은 값이다.
    //     잡몹의 휘두르기가 발끝 28~37 이므로 웅크리면 **머리 위로 지나간다.**
    //
    //     37 ~ 55   서 있는 머리     ← 여기에 닿으면 평타 두 대에 죽는다
    //     중단 띠 (28~37)            팔 20~36 · 몸통 18~37 에 닿는다
    //   ★ 27      웅크린 몸 전체    ← 그림도 판정도 같은 27
    //
    //   ★ 대신 잡몹의 물기(4~20)에는 그대로 맞는다. 웅크리기는 무적이 아니라
    //     **중단을 하단으로 바꾸는 거래**다.
    //
    //   ※ 세로만 줄이고 가로는 그대로다 — 웅크린다고 몸이 좁아지지는 않는다.
    //   (그림: 머리 27..12 · 몸통 13..4 · 다리 6..0)
    constexpr PartBox kBoxCrouch[Part_Count] = {
        { -10.0f, -27.0f,  10.0f, -12.0f },   // HEAD  — 크기는 그대로. 몸만 접힌다
        { -14.0f, -13.0f,  -3.0f,  -6.0f },   // L.ARM
        {   3.0f, -13.0f,  14.0f,  -6.0f },   // R.ARM
        { -11.0f, -13.0f,  11.0f,  -4.0f },   // TORSO
        { -10.0f,  -6.0f,  10.0f,   0.0f },   // LEGS
    };
