...
public:
    ...
    // ---- 위에 쌓여 있던 Scene 이 빠져서 **다시 맨 위가 되었을 때** ----
    //
    //   ★ 왜 필요한가:
    //     Pause 는 「닫히면 그냥 이어서 하면 되는」 것이라 이 훅이 없어도 됐다.
    //     그런데 Death 는 다르다 — 닫히는 순간 **아래 Scene 이 부활해야 한다.**
    //
    //     이게 없으면 아래 Scene 이 「내 Update 가 다시 불렸다 = 위가 사라졌다」로
    //     추측해야 하는데, 그 추측은 **틀린다.** 한 프레임에 틱이 여러 번 도는데
    //     Scene 전환은 프레임 끝에 한 번만 적용되므로, 전환을 요청한 그 프레임의
    //     다음 틱에서 「이미 닫혔다」로 오판한다.
    //
    //   ★ Replace / Clear 에서는 부르지 않는다.
    //     그때 빠지는 Scene 들은 곧 자기도 사라질 것이므로 「다시 맨 위가 되었다」가
    //     성립하지 않는다. Pop 만이 진짜로 아래를 되살린다.
    virtual void Resume(SceneContext&) {}
