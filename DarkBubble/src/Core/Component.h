...
//    바로 깨진다:
//
//                 부위파괴  무브셋  구르기  스태미나  강인도   AI
//        Player      O        O       O        O       O      X
//        Enemy       O        X       X        X       O      O
//
//    ★ 이 표는 실제로 **두 번 바뀌었다.** 처음에는 부위 파괴가 적에게만,
//      강인도가 플레이어에게만 있었다. 나중에 양쪽으로 퍼졌다.
//      상속 계층이었다면 그때마다 계층을 다시 짜야 했다 —
//      합성이라서 `Add<PartsComponent>` 한 줄씩이었다.
//
//    표가 이렇게 **비대칭이면서 움직인다**는 것이 요점이다.
//    「무브셋」을 Character 에 넣으면 적이 안 쓰는 코드를 물려받고,
//    Player 에만 넣으면 나중에 「무브셋이 있는 보스」를 못 만든다.
//    다중 상속으로 가면 다이아몬드가 생긴다.
//
//    합성이면 붙이면 끝이다. 계층을 다시 짜지 않는다.
