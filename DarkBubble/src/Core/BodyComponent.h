...
public:
    ...
    // 발끝을 **주어진 높이**에 놓고 멈춘다.
    //   ★ 부활 지점이 **지면 위에만 있는 것이 아니다** — 발판 위의 화톳불에서
    //     쉬고 죽으면 그 발판에서 일어나야 한다. 「바닥」을 맵의 지면 하나로
    //     두었을 때는 맞았지만, 높이가 둘이 되는 순간 전부 아래로 떨어졌다.
    void PlaceOnFloor(float y);

    // 공중에 놓는다. 속도를 지우고 **여기서부터 떨어진다.**
    //   ★ PlaceOnFloor 와 짝이다 — 저쪽은 「바닥에 놓고 멈춘다」.
    //     둘을 같이 두는 이유: 「어디에 놓는가」는 두 가지뿐이고,
    //     그 둘이 한눈에 보이면 셋째를 만들고 싶어질 때 먼저 의심하게 된다.
    void PlaceInAir(float y);

    // 맵의 지면에 놓는다. ★ 적의 리셋처럼 **지면이 답인 경우**에만 쓴다.
    void SnapToGround();
