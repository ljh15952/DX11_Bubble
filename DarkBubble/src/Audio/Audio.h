...
// ----------------------------------------------------------------------------
//  PanFromWorldX — **월드** x 를 스테레오 팬(-1 ~ +1)으로
//
//    ★ 전에는 `PanFromCanvasX(worldX)` 였다. 월드가 캔버스와 **같은 크기**인
//      동안에는 맞았지만, 맵이 넓어지는 순간 화면 오른쪽 끝에 붙은 소리가
//      **바로 옆에서 나도** 오른쪽 끝에서 들린다.
//      이름은 Canvas 인데 World 를 먹고 있었다 — 컴파일러는 둘 다 float 라 모른다.
//
//    그래서 **카메라 위치를 같이 받는다.** 부르는 쪽이 변환을 잊을 수 없다.
//
//    ★ PlayScene 안의 지역 헬퍼였는데 EnemyBrain 이 같은 것을 필요로 하게 되어
//      올렸다 — design.md §9 의 「두 번째 사용자가 생겼을 때」 그대로다.
//
//    오디오 쪽에 두는 이유: 「월드 좌표를 소리의 방향으로 바꾼다」는 것 자체가
//    오디오의 관심사다. 게임 코드가 -1..+1 이라는 XAudio2 의 규약을 알 필요가 없다.
// ----------------------------------------------------------------------------
